 ...
 * `Firestore` instance into the schema (`db.doc(...)`), so two databases cannot
 * share one. Both keys are held weakly: entries die with the schema or the
 * database they belong to.
 */
const operandSchemaCaches = new WeakMap<
  firestore.Firestore,
  WeakMap<DocumentSchema, Map<string, ZodAny>>
>();

const operandSchemaCache = (
  schema: DocumentSchema,
  db: firestore.Firestore,
): Map<string, ZodAny> => {
