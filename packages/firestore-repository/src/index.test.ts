...
it('FilterOperand', () => {
  ...
  expectTypeOf<FilterOperand<[number, 'a' | 'b'], 'not-in'>>().toEqualTypeOf<number | 'a' | 'b'>();
...
it('docPath', () => {
  ...
  expect(docPath(postsCollection, { postId: 123, authorId: 'abc' })).toBe('Authors/abc/Posts/123');
});
