...
import type { BuildSelection, ExpressionWithAlias } from './selection.js';
...
describe('BuildSelection', () => {
  ...
  describe('string paths', () => {
    ...
    it('preserves the Optional marker at the leaf of a dotted path', () => {
      expectTypeOf<BuildSelection<Schema, ['profile.gender']>>().toEqualTypeOf<{
        profile: MapType<{
          gender: LiteralType<['male', 'female']> & Optional;
        }>;
      }>();
    ...
    it('deep-merges siblings under a common parent', () => {
      expectTypeOf<BuildSelection<Schema, ['profile.age', 'profile.gender']>>().toEqualTypeOf<{
        profile: MapType<{
          age: DoubleType;
          gender: LiteralType<['male', 'female']> & Optional;
        }>;
    ...
    it('keeps the full subtree when a parent and its child are both selected', () => {
      ...
      expectTypeOf<BuildSelection<Schema, ['profile', 'profile.age']>>().toEqualTypeOf<{
        profile: MapType<{
          age: DoubleType;
          gender: LiteralType<['male', 'female']> & Optional;
        }>;
    ...
    it('merges three selections covering both top-level and nested', () => {
      expectTypeOf<BuildSelection<Schema, ['name', 'profile.age', 'rank']>>().toEqualTypeOf<{
        name: StringType;
        profile: MapType<{ age: DoubleType }>;
        rank: DoubleType;
      }>();
  ...
  describe('__name__', () => {
    it('is dropped when selected alone', () => {
      expectTypeOf<BuildSelection<Schema, ['__name__']>>().toEqualTypeOf<{}>();
    });

    it('is dropped when mixed with other selections', () => {
      expectTypeOf<BuildSelection<Schema, ['__name__', 'name']>>().toEqualTypeOf<{
        name: StringType;
      }>();
    });
  });
  ...
  describe('ExpressionWithAlias', () => {
    ...
    it('merges multiple aliases sharing a parent', () => {
      ...
      expectTypeOf<BuildSelection<Schema, [DeepAlias, AgeAlias]>>().toEqualTypeOf<{
  ...
  describe('mixed string and ExpressionWithAlias', () => {
    ...
    it('combines a string path and a top-level alias', () => {
      expectTypeOf<BuildSelection<Schema, ['name', ScoreAlias]>>().toEqualTypeOf<{
        name: StringType;
        score: DoubleType;
      }>();
    });
    ...
    it('deep-merges an alias into the same parent as a string path', () => {
      expectTypeOf<BuildSelection<Schema, ['profile.age', ProfileExtraAlias]>>().toEqualTypeOf<{
    ...
    it('deep-merges an alias into a subtree taken as a string', () => {
      ...
      expectTypeOf<BuildSelection<Schema, ['profile', ProfileExtraAlias]>>().toEqualTypeOf<{
        profile: MapType<{
          age: DoubleType;
          gender: LiteralType<['male', 'female']> & Optional;
          computed: StringType;
