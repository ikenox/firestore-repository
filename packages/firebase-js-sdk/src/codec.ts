...
const cache = (() => {
  ...
  return {
    ...
    operand: (schema: DocumentSchema, db: Firestore, key: string, build: () => ZodAny) =>
      memoize(encodersFor(schema, db).operands, key, build),

    /** The encoder for one cursor value, keyed by field path. */
    cursor: (schema: DocumentSchema, db: Firestore, fieldPath: string, build: () => ZodAny) =>
