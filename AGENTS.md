...
1. Create a branch and worktree: fetch the latest remote default branch (`git fetch origin main`), then create a new branch from `origin/main` and a git worktree for it under `.worktree/`, and switch to it. Choose an appropriate branch/worktree name based on the task description. Do not commit directly to main. Unless the user explicitly instructs otherwise (e.g., working on an existing branch, or skipping worktree creation), always follow this step.
